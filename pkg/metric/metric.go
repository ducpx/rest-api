...
func (metr *PrometheusMetrics) IncHits(status int, method string, path string) {
	metr.HitsTotal.Inc()
	metr.Hits.WithLabelValues(strconv.Itoa(status), method, path).Inc()
}

// ObserveResponseTime implements Metrics
func (metr *PrometheusMetrics) ObserveResponseTime(status int, method string, path string, observeTime float64) {
